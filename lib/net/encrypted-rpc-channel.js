...
EncryptedRpcChannel.prototype._onNewSessionCreated = function (newSession) {
    ...
    logger.debug("New session has been created by Telegram: %s", newSession.toPrintable());
...
EncryptedRpcChannel.prototype._sendAck = function (msg) {
    ...
    logger.debug("Scheduling ACK for msg_id=%s", msg._messageId);
...
EncryptedRpcChannel.prototype._onMsgsAck = function (msgIds) {
    logger.debug("The server sent a acknowledge for the followings [%s]", msgIds);
...
EncryptedRpcChannel.prototype._onBadServerSalt = function (badServerSalt) {
    ...
    logger.debug("Wrong server_salt in message [%s]", badMsgId);
...
EncryptedRpcChannel.prototype.sendMessagesAck = function () {
    ...
    if (msgsAck) {
        logger.debug("Send the ack to the server: %s", msgsAck.toPrintable());
        ...
        this.constructor.super_.prototype._call.call(this, reqMsg, function (ex, result, duration) {
            try {
                if (ex) {
                    ...
                    logger.debug('Call of \'%s\' took %sms', reqMsg.body.getTypeName(), duration);
